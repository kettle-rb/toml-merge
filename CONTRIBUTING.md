...
For a quick starting point, this repository’s `mise.toml` defines the shared defaults, and `.env.local` can override them locally. Copy `.env.local.example` to `.env.local`, use `KEY=value` lines, and either activate `mise` in your shell or run commands through `mise exec -C /home/pboling/src/kettle-rb/toml-merge -- ...`.
...
[🧪build]: https://github.com/kettle-rb/toml-merge/actions
[🤝conduct]: https://gitlab.com/kettle-rb/toml-merge/-/blob/main/CODE_OF_CONDUCT.md
